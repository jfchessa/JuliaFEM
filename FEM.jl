...
function nodearray2d(corners, n1, n2=n1)
    ...
    nn = Int(n1*n2)
	x = zeros(s,nn)
	xi = linspace(-1, 1, Int(n1))
	eta = linspace(-1, 1, Int(n2))
...
function test2()
	node = [0.0 1.0 2.0 0.0 1.0 2.0;0.0 0.0 0.0 1.0 1.0 1.0]
	element = FEM.Quad4D2([1 2;2 3;5 6; 4 5])
	node1 = FEM.Meshing.nodearray2d([0.0 2.0 2.0 0.0;0.0 0.0 1.0 1.0], 5, 3)

	nn = size(node,2)
	ne = FEM.numelem(element)
	nne = FEM.elemnne(element)
	sdim = FEM.elemsdim(element)
	edim = FEM.elemedim(element)
	ndofpn = sdim
	vdim = Dict(1=>1, 2=>3, 3=>6)[sdim]
	qtype = FEM.elemqtype(element)
	(qpt, qwt) = FEM.quadrule(qtype, sdim, 2)
	nqpt = length(qwt)
	dNxi = zeros(Float64, nne, edim, nqpt)
	FEM.dshape_quad4!(dNxi, qpt)
	dNdx = zeros(Float64, nne, sdim)
	sctr = zeros(Int32, nne*ndofpn)
	Bmat = zeros(Float64, vdim, nne*ndofpn)
	nu = 0.3
	Yg = 10.0e6
	Cmat = Yg/(1-nu^2)*[1 -nu 0;-nu 1 0; 0 0 1-nu]
	Kmat = spzeros(Float64, nn*ndofpn, nn*ndofpn)
	for e = 1:ne
		econn = element.conn[:,e]
		coord = node[:,econn]
		FEM.setsctr!(sctr, econn, nne, ndofpn)
		Ke =zeros(Float64, nne*ndofpn, nne*ndofpn)
		for q = 1:nqpt
			detj = FEM.gradbasis!(dNdx, dNxi[:,:,q], coord, nne, sdim, edim)
			FEM.formbmat!(Bmat, dNdx, nne, sdim, vdim)
			Ke += transpose(Bmat)*Cmat*Bmat*(qwt[q]*detj)
			println("quadrature point ", q)
		end
		println("sctr = ", sctr)
		Kmat[sctr, sctr] = Kmat[sctr, sctr] + Ke
	end

	F = zeros(Float64, nn*ndofpn)
	F[5] = 100.0
	F[11] = 100.0

	ifix = [1, 2, 7]

	d = FEM.fesolve!(Kmat, F, ifix)

	output = FEM.IO.output_mesh("ex1.vtu", node, element)
